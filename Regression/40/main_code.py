...
def load_fashion_mnist_multiclass(selected_classes=[0, 1], n_train=100, n_test=1000, n_val=200, pca_dim=4, seed=42):
    """
    Load Fashion-MNIST, select subset of classes, downsample, Apply PCA.
    """
    # Downsample 28x28 -> 7x7 to speed up PCA and reduce noise
    transform = transforms.Compose([
        transforms.ToTensor(),
        transforms.Lambda(lambda x: torch.nn.functional.avg_pool2d(x, kernel_size=4)) 
    ])

    # Load FashionMNIST instead of CIFAR10 or MNIST
    f_mnist_train = FashionMNIST(root="./data", train=True, download=True, transform=transform)
    f_mnist_test = FashionMNIST(root="./data", train=False, download=True, transform=transform)
    ...
    X_train_full, y_train_full = filter_classes(f_mnist_train)
    X_test_full, y_test_full = filter_classes(f_mnist_test)

    # One-hot encoding
    encoder = OneHotEncoder(sparse_output=False, categories=[selected_classes])
    y_train_oh = encoder.fit_transform(y_train_full.reshape(-1, 1))
    y_test_oh = encoder.transform(y_test_full.reshape(-1, 1))

    # Map 0 -> -1
    y_train_oh = np.where(y_train_oh == 0, -1, 1)
    y_test_oh = np.where(y_test_oh == 0, -1, 1)
    ...
    # Normalize to [-pi/2, pi/2] for quantum embedding
    X_all = np.vstack([X_train_pca, X_test_pca])
    X_all_normalized = normalize_to_minus_pi_pi(X_all)
    X_train_pca = X_all_normalized[:len(X_train_pca)]
    X_test_pca = X_all_normalized[len(X_train_pca):]

    # Subsample
    np.random.seed(seed)
    idx_train = np.random.choice(len(X_train_pca), n_train + n_val, replace=False)
    idx_test = np.random.choice(len(X_test_pca), n_test, replace=False)

    X_train = torch.tensor(X_train_pca[idx_train[:n_train]], dtype=torch.float32)
    y_train = torch.tensor(y_train_oh[idx_train[:n_train]], dtype=torch.float32)
    X_val = torch.tensor(X_train_pca[idx_train[n_train:]], dtype=torch.float32)
    y_val = torch.tensor(y_train_oh[idx_train[n_train:]], dtype=torch.float32)
    X_test = torch.tensor(X_test_pca[idx_test], dtype=torch.float32)
    y_test = torch.tensor(y_test_oh[idx_test], dtype=torch.float32)
...
def train_model(model, X_train, y_train, X_val, y_val, X_test, y_test,
    ...
    lr=0.01  # learning rate depends on number of parameters (more params need smaller lr)
    ...
    with torch.no_grad():
        ...
        # Note: argmax is for accuracy (classification). For regression we look at Loss.
        # However, to keep code running without crash, we calculate dummy accuracy or ignore it.
        # For synthetic regression, 'accuracy' isn't really defined, but we'll leave logic to avoid crash.
        ...
        print(f'Train acc.: {train_acc:.4f}, Test acc.: {test_acc:.4f} (Ignore Acc for Regression)')
...
# --- MODIFICATION 2: Synthetic Data Settings ---
noise_std = 0.5
input_dim = n_qubits
K = 3 # Number of outputs for multiclass regression
output_dim = K

# Create random weights for the synthetic target function
# Shape: (input_dim, output_dim)
weights_synthetic = np.random.randn(input_dim, output_dim)
...
reps = 10  # Reduced reps for quicker testing, set back to 10 if needed
...
for r in range(reps):
    ...
    # --- MODIFICATION 3: Load Synthetic Data instead of Fashion MNIST ---
    X_train, y_train, X_test, y_test = load_synthetic_multiclass_linear(
        weights=weights_synthetic, 
        input_dim=input_dim, 
        n_train=n_train, 
        output_dim=K, 
        n_test=1000, 
    ...
    # Synthetic loader doesn't return validation, so we reuse X_test for validation monitoring
    ...
    for depth in depths:
        ...
        test_loss, train_loss, test_acc, train_acc = train_model(model, X_train, y_train, X_val, y_val, X_test, y_test, loss=loss, batch_size=batch_size,lr=lr,epochs=epochs)
        ...
        # see how large / small the largest / smallest test output is after training
        y_pred = model(X_test)
