#%%
"""Import libraries and set system size (n_qubits)"""
...
#%%
"""Define some functions we need for analysis later"""
...
def functional_hvp(model, X, loss_fn, y, v):
    model.eval()
    ...
    idx = 0
    for p in params:
        v_split.append(v[idx:idx + p.numel()].view_as(p))
        idx += p.numel()
    ...
    directional = sum(
        (g * v_i).sum() for g, v_i in zip(grads, v_split)
    )

    # second derivative: ONLY hits f, not ℓ
    ...
    Hv = torch.cat([h.reshape(-1) for h in hv]) / X.shape[0]

    return Hv

def lanczos_lambda_max(hvp_fn, dim, k=20, device="cpu"):
    """
    Estimate largest eigenvalue of H using Lanczos.

    Args:
        hvp_fn: function v -> H v
    ...
    q = torch.randn(dim, device=device, dtype=torch.float32)
    q = q / q.norm()
    ...
    return eigvals[-1].item()  # largest


#%%
"""Define different functions for data"""
...
"""Here we define the quantum model and wrap it in a torch class"""
...
def train_model(model, X_train, y_train, X_val, y_val, X_test, y_test,
    ...
    return test_loss, train_loss_final, test_acc, train_acc, model

"""This is the most important part: here we define all hyperparameters (number of classes, number of training data,...)"""
...
print(f'Peak expected at n_params e n= {peak_pos}')
...
#%% This cell is the actual training reps-times (takes quite long to execute)"""
...
model_arr = [[]for r in range(reps)]
lambda_max_H_f_arr = [[]for r in range(reps)]
...
for r in range(reps):
    ...
    X_train, y_train, X_test, y_test, X_val, y_val, K = load_mnist1d_multiclass(selected_classes=selected_classes, n_train=n_train, n_test=1000, n_val=0, pca_dim=pca_dim, seed=seed)
    ...
    for depth in depths:
        ...
        test_loss, train_loss, test_acc, train_acc, trained_model = train_model(model, X_train, y_train, X_test, y_test, X_test, y_test, loss=loss, batch_size=batch_size,lr=lr,epochs=epochs)
        model_arr[r].append(trained_model)
        ...
        def hvp_fn(v):
            return functional_hvp(model, X_train, loss, y_train, v)
        
        
        lambda_max_H_f = lanczos_lambda_max(
            hvp_fn=hvp_fn,
            dim=params,
            k=30,
        ...
        print(f'Lambda_max of H_f {lambda_max_H_f:.2e}')
        lambda_max_H_f_arr[r].append(lambda_max_H_f)
    ...
    # save lambda_max_H_f and total_param_arr after each completed repetition
    lambda_max_H_f_save_file = "lambda_max_H_f_runs.npz"
    np.savez(
        lambda_max_H_f_save_file,
        total_param_arr=np.array(total_param_arr, dtype=int),
        lambda_max_H_f_arr=np.array(lambda_max_H_f_arr[:r+1], dtype=float),
        depths=np.array(depths, dtype=int),
        reps_completed=r + 1
    )
    print(f"Saved lambda_max_H_f_arr and total_param_arr to {lambda_max_H_f_save_file}")
...
#%% Plot average test error (and smallest eigenvalues) over repetitions"""
...
# %% check approximations of H_f

trained_model_arr = model_arr[0]
model = trained_model_arr[0]
        
params = sum(p.numel() for p in model.parameters())

# %%

lambda_min_C_f = min_eig_arr[0]
lambda_max_H_f = lambda_max_H_f_arr[0]

#plt.plot(lambda_min_C_f)
plt.plot(total_param_arr, lambda_max_H_f, marker='o')
plt.show()

#%%
plt.plot(np.array(lambda_min_C_f)+np.array(lambda_max_H_f), marker='o')
plt.ylabel('lambda_min_C_f + lambda_max_H_f')
#plt.yscale('log')
plt.legend()
plt.show()
...
# %%
plt.plot(1/np.array(test_loss_arr), marker='o')
plt.ylabel('1 / test error')
#plt.yscale('log')
plt.legend()
plt.show()
