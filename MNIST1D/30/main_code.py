...
def load_mnist_multiclass(selected_classes=[0, 1], n_train=20, n_test=1000, n_val=100, pca_dim=10, seed=42):
    """
    Load MNIST, select subset of classes, downsampled, and return tensor datasets.
    Labels are one-hot encoded for use with MSE loss.
    """
    #transform = transforms.Compose([transforms.ToTensor()])
    # Transform to convert to tensor and downsample from 28x28 to 7x7
    transform = transforms.Compose([
        transforms.ToTensor(),
        #transforms.Normalize((0.1307,), (0.3081,)),
        transforms.Lambda(lambda x: torch.nn.functional.avg_pool2d(x, kernel_size=4))  # 28x28 -> 7x7
    ])
    ...
    X_train_full, y_train_full = filter_classes(mnist_train)
    X_test_full, y_test_full = filter_classes(mnist_test)
